...
MsSQL.prototype.executeSQL = function (sql, params, options, callback) {
  ...
  //       // Float number
  //       sql = sql.replace('@param' + (i + 1), params[i]);
  //     } else if (typeof params[i] === 'boolean') {
  //       sql = sql.replace('@param' + (i + 1), params[i] == true ? 1 : 0);
  //     } else if (typeof params[i] === 'string') {
  //       // Reverting this 
  //       // sql = sql.replace('@param' + (i + 1), "'" + params[i] + "'") //? params[i].replace("'", "''") : null);
  //       // console.log('params', params[i])
  //       // console.log('i', i)
  //       sql = sql.replace('@param' + (i + 1), params[i] === null ? `null` : `'${params[i].replace("'", "''")}'`);
  //     } else {
  //       sql = sql.replace('@param' + (i + 1), params[i]);
  //     }
  //   }
  ...
  if (Array.isArray(params) && params.length > 0) {
    for (var i = 0, n = params.length; i < n; i++) {
      if (typeof params[i] === 'string') {
        sql = sql.replace('@param' + (i + 1), params[i] === 'null' ? null : `'${params[i].replace(/'/g, "''")}'`);
      } else if (typeof params[i] === 'boolean') {
        sql = sql.replace('@param' + (i + 1), params[i] == true ? 1 : 0);
      } else {
        sql = sql.replace('@param' + (i + 1), params[i]);
  ...
  //   for (var i = 0, n = params.length; i < n; i++) {
  //     console.log('@param' + (i + 1), params[i])
  //     sql = sql.replace('@param' + (i + 1), params[i]);
  //   }
  // }
 ...
 * @param {Object} data Array of model instance data
 * @param {Object} options The options
 * @param {Function} [cb] The callback function
 */
MsSQL.prototype._createMany = function (model, data, options = {}, cb) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    for (var j = 0; j < validKeys.length; j++) {
      ...
      //valueData.push(this.toColumnValue(props[key], data[i][key]));
  ...
  this.execute(sql, valueData, options, function (err, info) {
